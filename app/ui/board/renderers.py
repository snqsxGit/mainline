...
class BoardRenderer:
    ...
    def __init__(self, theme: BoardTheme) -> None:
        self._theme = theme
        self.selected_color = QColor(246, 246, 105, 118)
        self.destination_color = QColor(20, 85, 30, 88)

    def set_theme(self, theme: BoardTheme) -> None:
        """Update theme colors used for square painting."""
        self._theme = theme

    def paint_squares(self, painter: QPainter, square_rects: list[list[QRectF]]) -> None:
    ...
    def paint_selection(self, painter: QPainter, rect: QRectF | None) -> None:
        """Paint the selected square overlay."""
        if rect is not None:
            painter.fillRect(rect, self.selected_color)

    def paint_destinations(self, painter: QPainter, rects: list[QRectF]) -> None:
        """Paint subtle legal-destination dots."""
        painter.setPen(Qt.PenStyle.NoPen)
        painter.setBrush(self.destination_color)
        for rect in rects:
...
class PieceRenderer:
    ...
    def draw_piece(self, painter: QPainter, piece: chess.Piece, rect: QRectF) -> None:
